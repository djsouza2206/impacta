   ...
   For Each NomeProj In NomeProjeto
      ...
      ArqDir = Dir(PathFull & "*.MYS", vbNormal)
         ...
         For K = 1 To NroArq                             ' - 4  ==> TIRA A EXTENSAO DO ARQUIVO
            If Dir("C:\MYSQL\DATA\TABUM\" & Mid(ArqMySql(K), 1, Len(Trim(ArqMySql(K))) - 4) & ".*", vbNormal) <> "" Then
               CmdSql = "FLUSH TABLES"
               ...
               Kill "C:\MYSQL\DATA\TABUM\" & Mid(ArqMySql(K), 1, Len(Trim(ArqMySql(K))) - 4) & ".*"
            End If
...
'  TABELA DE CONTROLE FINANCEIRO
   CmdSql = "CREATE TABLE IF NOT EXISTS CONTROLE_FINANCEIRO(" & vbCr
   CmdSql = CmdSql & "        PROJETO                 VARCHAR(20)    NOT NULL," & vbCr
   CmdSql = CmdSql & "        TIPO                    VARCHAR(20)    NOT NULL," & vbCr
   CmdSql = CmdSql & "        CATEG                   VARCHAR(50)    NOT NULL," & vbCr
   CmdSql = CmdSql & "        SUB_CATEG               VARCHAR(50)    NOT NULL," & vbCr
   CmdSql = CmdSql & "        DATA                    DATE           NOT NULL," & vbCr
   CmdSql = CmdSql & "        DESCRICAO               VARCHAR(255)   DEFAULT ' '," & vbCr
   CmdSql = CmdSql & "        VALOR                   DECIMAL(14,2)  NOT NULL," & vbCr
   CmdSql = CmdSql & "        STATUS                  VARCHAR(01)    DEFAULT ' '" & vbCr
   CmdSql = CmdSql & ")TYPE=MYISAM"
   CMySql.Executa CmdSql, True
      
   CmdSql = "CREATE TABLE IF NOT EXISTS TIPO_MOVIMENTACAO("
   CmdSql = CmdSql & "        TIPO                    VARCHAR(20) NOT NULL," & vbCr
   CmdSql = CmdSql & "        CATEG                   VARCHAR(50) NOT NULL,"
   CmdSql = CmdSql & "        SUB_CATEG               VARCHAR(50) NOT NULL,"
   CmdSql = CmdSql & "        REGRA                   VARCHAR(50) NOT NULL"
   CmdSql = CmdSql & ")TYPE=MYISAM"
   CMySql.Executa CmdSql, True
